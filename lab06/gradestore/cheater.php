...
<html lang="en">
	...
	<body>
		...
		if (!isset($_POST["name"], $_POST["id"],$_POST["course"], $_POST["cc_num"], $_POST["cc_type"])
			|| empty($_POST["name"]) || empty($_POST["id"]) || empty($_POST["cc_num"])) {
		?>
		<!-- Ex 4 :
	...
	} elseif (!preg_match('/^\d{16}$/', $_POST["cc_num"])
			|| (($_POST["cc_type"] == "visa") && (!preg_match('/^4/', $_POST["cc_num"])))
			|| (($_POST["cc_type"] == "master") && (!preg_match('/^5/', $_POST["cc_num"])))) {
		?>
